...
impl DrmSurface {
    ...
    /// Ask for page flips that do not wait for the vertical retrace.
    ///
    /// LineXinBar patch. This is what `wp_tearing_control_v1` asks a compositor
    /// for, and there is otherwise no way to express it: the atomic surface
    /// always commits at the retrace.
    ///
    /// A hint, in both directions. The driver may refuse an immediate flip for
    /// a given frame — amdgpu does whenever the commit changes anything but the
    /// primary plane's address — and a refused flip is retried at the retrace
    /// rather than dropped, so the worst case of asking is the behaviour of
    /// never having asked. Has no effect on the legacy surface.
    pub fn set_tearing(&self, tearing: bool) {
        if let DrmSurfaceInternal::Atomic(surf) = &*self.internal {
            surf.tearing.store(tearing, std::sync::atomic::Ordering::SeqCst);
        }
    }
