...
pub struct AtomicDrmSurface {
    ...
    /// LineXinBar patch: whether the next page flip should be immediate rather
    /// than waiting for the retrace. See `DrmSurface::set_tearing`.
    pub(super) tearing: AtomicBool,
...
impl AtomicDrmSurface {
    ...
    pub fn page_flip<'a>(
        ...
        // LineXinBar patch: tearing. See `DrmSurface::set_tearing`.
        let tearing = self.tearing.load(Ordering::SeqCst);
        if tearing {
            flags |= AtomicCommitFlags::PAGE_FLIP_ASYNC;
        }
        let request = req.build()?;
        let mut res = self.fd.atomic_commit(flags, request.clone()).map_err(|source| {
            Error::Access(AccessError {
        ...
        // A driver may refuse an immediate flip for this particular frame —
        // amdgpu does when the flip would change anything but the primary
        // plane's address, and the modifier has to match what is already
        // scanned out. Tearing is a hint, so a refusal is not an error: retry
        // the same request at the retrace rather than dropping the frame.
        if res.is_err() && tearing {
            trace!("Immediate page flip refused; retrying at the retrace");
            res = self
                .fd
                .atomic_commit(flags - AtomicCommitFlags::PAGE_FLIP_ASYNC, request)
                .map_err(|source| {
                    Error::Access(AccessError {
                        errmsg: "Page flip commit failed",
                        dev: self.fd.dev_path(),
