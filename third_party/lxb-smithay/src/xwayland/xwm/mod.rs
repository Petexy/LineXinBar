    ...
    D: 'static,
{
    ...
    match event {
        ...
        // `_NET_ACTIVE_WINDOW` is deliberately not written here. Focus events
        // are selected on the *root*, so `n.event` is the root window on every
        // one of them — this wrote "the root window is the active window" into
        // the property for the whole life of the session, on both edges, and a
        // client that reads it (Wine does, so every game under Proton does)
        // concluded it was in the background while it was the only thing on
        // screen. The compositor names the active window itself, from what it
        // knows about which application the user can see; see
        // `LxbState::name_the_active_x11_window`.
