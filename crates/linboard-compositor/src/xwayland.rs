...
/// geometry Linboard configures for application windows. A client can change
...
impl LinboardState {
    ...
    pub(crate) fn raise_window(&mut self, window: &Window, activate: bool) {
            ...
            .is_some_and(|surface| surface.is_override_redirect())
        {
            if activate {
                ...
                let windows: Vec<Window> = self.linboard.space.elements().cloned().collect();
        ...
        self.linboard.space.raise_element(window, activate);
    ...
    pub(crate) fn sync_x11_raise(&mut self, window: &Window) {
        ...
        let Some(xwm) = self.linboard.xwm.as_mut() else {
            return;
        };
    ...
    fn restack_override_redirect(&mut self, window: &Window, above: Option<u32>) {
        let mut order: Vec<Window> = self.linboard.space.elements().cloned().collect();
        ...
        for element in order {
            self.linboard.space.raise_element(&element, false);
        }
    }
}
...
impl XWaylandShellHandler for LinboardState {
    fn xwayland_shell_state(&mut self) -> &mut XWaylandShellState {
        &mut self.linboard.xwayland_shell_state
    }
...
impl XWaylandKeyboardGrabHandler for LinboardState {
...
impl XwmHandler for LinboardState {
        ...
        if !surface.is_override_redirect() {
            if let Some(window) = self.window_for_x11(&surface) {
                ...
                if !window_is_x11_chrome(&window) {
                    // Linboard's regular application windows are always
    ...
    fn send_selection(
        ...
        match selection {
            SelectionTarget::Clipboard => {
                if let Err(err) =
                    request_data_device_client_selection(&self.linboard.seat, mime_type, fd)
                {
            ...
            SelectionTarget::Primary => {
                if let Err(err) =
                    request_primary_client_selection(&self.linboard.seat, mime_type, fd)
                {
    ...
    fn cleared_selection(&mut self, _xwm: XwmId, selection: SelectionTarget) {
        match selection {
            SelectionTarget::Clipboard
                if current_data_device_selection_userdata(&self.linboard.seat).is_some() =>
            {
                clear_data_device_selection(&self.linboard.display_handle, &self.linboard.seat)
            }
            SelectionTarget::Primary
                if current_primary_selection_userdata(&self.linboard.seat).is_some() =>
            {
                clear_primary_selection(&self.linboard.display_handle, &self.linboard.seat)
            }
            _ => {}
        }
    }
    ...
    fn disconnected(&mut self, xwm: XwmId) {
        if self.linboard.xwm.as_ref().map(X11Wm::id) != Some(xwm) {
            return;
        }
        ...
        self.linboard.fatal_error = Some(
            "private XWayland connection closed; restart the complete Linboard session".into(),
        );
...
smithay::delegate_xwayland_shell!(LinboardState);
smithay::delegate_xwayland_keyboard_grab!(LinboardState);
