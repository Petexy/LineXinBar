...
pub fn init(
    ...
    for index in 0..count {
        let window = WindowBuilder::new()
            .title(&format!("Linboard (virtual output {})", index + 1))
        ...
        let global = output.create_global::<LinboardState>(&dh);
    ...
    if !dmabuf_formats.is_empty() {
        let global = state
            .linboard
            .dmabuf_state
            .create_global::<LinboardState>(&state.linboard.display_handle, dmabuf_formats);
        if let super::Backend::X11(x11) = &mut state.backend {
            x11.dmabuf_global = Some(global);
        }
    }

    state.linboard.pointer_location = (size.0 as f64 / 2.0, size.1 as f64 / 2.0).into();
...
fn handle_event(state: &mut LinboardState, event: X11Event) {
    match event {
...
fn render_all(state: &mut LinboardState) {
...
fn render_output(state: &mut LinboardState, window_id: u32) -> anyhow::Result<()> {
    let super::Backend::X11(x11) = &mut state.backend else {
        return Ok(());
    };
    ...
    cursor.status = state.linboard.cursor_status.clone();
