...
//! exports from the config before any backend starts. Linboard bundles the
...
use crate::render::LinboardRenderElement;
        ...
        match &self.status {
            ...
            CursorImageStatus::Named(icon) => {
                ...
                match MemoryRenderBufferRenderElement::from_buffer(
                    ...
                    Ok(element) => vec![LinboardRenderElement::Memory(element)],
                    Err(err) => {
...
pub fn export_cursor_environment(theme: Option<&str>, size: Option<u32>) {
    ...
    if let Some(dir) = std::env::var_os("LINBOARD_DATA_DIR") {
        paths.push(std::path::PathBuf::from(dir).join("icons"));
    }
    if let Ok(exe) = std::env::current_exe() {
        // <prefix>/bin/linboard -> <prefix>/share/icons, and
        // target/<profile>/linboard -> <checkout>/share/icons.
        for up in [1, 2] {
