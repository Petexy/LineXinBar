...
fn keymap(alphabet: &[Stroke], spare: bool) -> String {
    ...
    text.push_str("xkb_keycodes \"linboard\" {\n");
    ...
    text.push_str("};\n");
    text.push_str("xkb_types \"linboard\" { include \"complete\" };\n");
    text.push_str("xkb_compat \"linboard\" { include \"complete\" };\n");
    text.push_str("xkb_symbols \"linboard\" {\n");
...
fn keymap_file(text: &str) -> std::io::Result<(OwnedFd, u32)> {
    ...
    let raw = unsafe { libc::memfd_create(c"linboard-keymap".as_ptr(), libc::MFD_CLOEXEC) };
    if raw < 0 {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Typed {
    /// Move the cursor over the board.
    Move(Move),
    /// Press the key under the cursor.
    Press,
    /// Put the board away.
    Close,
    /// None of the board's business: type it into the application, exactly as
    /// though the matching key on the board had been pressed.
    Send(Stroke),
    /// Nothing the board can express.
    Ignored,
}

impl Typed {
    /// Whether holding the key down should go on doing this.
    ///
    /// Directions and characters, and nothing else. A Press or a Close that
    /// fired thirty times a second because a finger rested on it would be a
    /// keyboard that types by accident, and a board that puts itself away.
    fn repeats(self) -> bool {
        matches!(self, Typed::Move(_) | Typed::Send(_))
    }
}

/// Read a keysym as a board control, or as something to type.
///
/// The set of controls is deliberately tiny — the four arrows, Enter, Escape —
/// because every other key has to stay a key. The bar reads `w`/`a`/`s`/`d`
/// and `h`/`j`/`k`/`l` as directions, and doing the same here would leave a
/// keyboard unable to type half the alphabet into the field the board exists
/// to fill.
///
/// Space is left alone for the same reason, though it is *accept* everywhere
/// else in the shell: a space bar that refused to make a space would be the
/// most surprising key on the board.
pub fn interpret(keysym: Keysym) -> Typed {
    match keysym {
        Keysym::Left | Keysym::KP_Left => Typed::Move(Move::Left),
        Keysym::Right | Keysym::KP_Right => Typed::Move(Move::Right),
        Keysym::Up | Keysym::KP_Up => Typed::Move(Move::Up),
        Keysym::Down | Keysym::KP_Down => Typed::Move(Move::Down),
        Keysym::Return | Keysym::KP_Enter => Typed::Press,
        Keysym::Escape => Typed::Close,
        _ => match stroke_for(keysym) {
...
fn stroke_for(keysym: Keysym) -> Option<Stroke> {
...
struct Grab {
    ...
    modifiers: Modifiers,
    /// How long a key must be held before it repeats, and how often after
    /// that. `None` where the compositor asked for no repeat at all.
    repeat: Option<(Duration, Duration)>,
    /// The key being held down, and when it next fires.
    held: Option<(u32, Instant)>,
}
...
impl Grab {
    fn new(keyboard: ZwpInputMethodKeyboardGrabV2) -> Self {
        Self {
            ...
            // Until the compositor says otherwise. It sends `repeat_info` with
            // the grab, so this is only the state between the two.
            repeat: None,
            held: None,
        }
    }
    ...
    /// `rate` is in keys per second, and zero disables repeat; `delay` is in
    /// milliseconds.
    fn set_repeat(&mut self, rate: i32, delay: i32) {
        self.repeat = (rate > 0).then(|| {
            (
...
pub struct Osk {
    ...
    /// When it was last opened, for the slide up from the bottom edge.
...
impl Osk {
    ...
    /// Seconds since it was opened, for the slide up.
    pub fn age(&self) -> f32 {
        self.opened_at
    ...
    fn set_focused(&mut self, focused: bool) -> bool {
        ...
        if focused {
            // The field itself asks for the keyboard, once. Closing it leaves
            // the hint in its place until the cursor moves on.
            if !self.offered {
                self.open();
            } else if self.open {
    ...
    /// do about it.
    ///
    /// Releases are only bookkeeping: they stop the repeat. Nothing the board
    /// does is held down.
    pub fn key(&mut self, keycode: u32, pressed: bool, now: Instant) -> Typed {
        let Some(grab) = self.grab.as_mut() else {
            return Typed::Ignored;
        };
        if !pressed {
            if grab.held.is_some_and(|(held, _)| held == keycode) {
                grab.held = None;
            }
            return Typed::Ignored;
        }
        let Some(keysym) = grab.keysym(keycode) else {
            return Typed::Ignored;
        };
        let typed = interpret(keysym);
        grab.held = match grab.repeat {
            Some((delay, _)) if typed.repeats() => Some((keycode, now + delay)),
            _ => None,
        };
        typed
    }

    /// Whatever key is being held down, if it is due to fire again.
    ///
    /// The compositor delivers one event per press and leaves repeat to
    /// whoever took the grab, which for a keyboard being *navigated* matters
    /// more than usual: holding Right to cross the board is how anyone with a
    /// keyboard will expect to reach the far side of it.
    pub fn repeat(&mut self, now: Instant) -> Typed {
        let Some(grab) = self.grab.as_mut() else {
            return Typed::Ignored;
        };
        let (Some((keycode, due)), Some((_, interval))) = (grab.held, grab.repeat) else {
            return Typed::Ignored;
        };
        if now < due {
            return Typed::Ignored;
        }
        grab.held = Some((keycode, now + interval));
        grab.keysym(keycode)
            .map(interpret)
...
mod tests {
    ...
    /// What the user asked for, in the words they asked for it in: the arrows
    /// move the selected key, and Enter presses it rather than reaching the
    /// application as an Enter.
    #[test]
    fn the_arrows_drive_the_board_and_enter_presses_the_key_under_them() {
        assert_eq!(interpret(Keysym::Left), Typed::Move(Move::Left));
        assert_eq!(interpret(Keysym::Right), Typed::Move(Move::Right));
        assert_eq!(interpret(Keysym::Up), Typed::Move(Move::Up));
        assert_eq!(interpret(Keysym::Down), Typed::Move(Move::Down));
        // The keypad's, which are a different keysym and the same intent.
        assert_eq!(interpret(Keysym::KP_Down), Typed::Move(Move::Down));

        assert_eq!(interpret(Keysym::Return), Typed::Press);
        assert_eq!(interpret(Keysym::KP_Enter), Typed::Press);
        assert_eq!(interpret(Keysym::Escape), Typed::Close);
    }

    /// And the other half, which matters just as much: with the board holding
    /// the physical keyboard, everything that is not one of those six controls
    /// has to reach the application anyway, or opening a keyboard would stop
    /// the user typing.
    #[test]
    fn every_other_key_is_still_typed_into_the_application() {
        ...
        // The bar reads `w`/`a`/`s`/`d` and `h`/`j`/`k`/`l` as directions. The
        // board must not, or half the alphabet would steer it instead of being
        // typed into the field it exists to fill.
        for keysym in [Keysym::w, Keysym::a, Keysym::s, Keysym::d, Keysym::h] {
            assert!(
                matches!(interpret(keysym), Typed::Send(Stroke::Char(_))),
                "{keysym:?} steered the board instead of typing"
            );
        }

        // Everything the board can send is something the board can be sent.
        let alphabet = alphabet();
        for keysym in [Keysym::q, Keysym::Q, Keysym::BackSpace, Keysym::F1] {
        ...
        // And a key with no character and no place on the board goes nowhere
        // rather than somewhere wrong.
        assert_eq!(interpret(Keysym::Print), Typed::Ignored);
        assert_eq!(interpret(Keysym::Shift_L), Typed::Ignored);
    }

    /// Only what can usefully be held down repeats. A Press that fired thirty
    /// times a second would type by accident, and a Close would put the board
    /// away under the finger that was still steering it.
    #[test]
    fn only_directions_and_characters_repeat_while_held() {
        assert!(Typed::Move(Move::Left).repeats());
        assert!(Typed::Send(Stroke::BACKSPACE).repeats());
        assert!(!Typed::Press.repeats());
        assert!(!Typed::Close.repeats());
        assert!(!Typed::Ignored.repeats());
    }
