...
//!
//! The movement itself goes to the compositor over `linboard_shell_v1`: only
...
const PREAMBLE: &str = "\
# Per-application settings, written by the Linboard shell.
#
...
/// `$XDG_CONFIG_HOME/linboard/apps.toml`, beside the compositor's own config.
fn prefs_path() -> Option<PathBuf> {
    ...
    Some(config.join("linboard").join("apps.toml"))
}
