...
fn environment(mirrored: vec3<f32>) -> vec3<f32> {
    ...
    return ambient + vec3<f32>(1.0, 0.98, 0.93) * key * 26.0;
}
...
fn fs_quad(in: QuadOut) -> @location(0) vec4<f32> {
    ...
    if (radius > 0.0) {
        ...
        if (in.shape.y <= 0.0 && (thickness > 0.0 || gloss > 0.0)) {
            ...
            // The surface, in three dimensions at last: tilted outwards by the
            // bevel, level over the face. Everything below is this vector.
            let surface = normalize(vec3<f32>(outward * bevel_slope(inset), 1.0));
            ...
            if (gloss > 0.0) {
                ...
                glass = mix(glass, environment(reflect(vec3<f32>(0.0, 0.0, -1.0), surface)), lit);
