...
//! refuses the build when the two have drifted apart. `lxb-desktop --version`
...
//!
//! The compositor beside it carries the same check rather than one crate
//! standing for both, because the two halves are built apart as well as
//! together: the compositor-only package runs `cargo build -p lxb-compositor`,
//! with this crate nowhere in the graph to be checked.
